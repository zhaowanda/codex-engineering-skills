...
PHRASE_REPLACEMENTS_ZH = {
    ...
    "review route consumers before implementation": "实施前检查路由消费方",
    ...
    "confirm only unless implementation proves contract change is required": "默认仅确认契约影响，除非实现证明必须变更契约",
    ...
    "write through": "通过",
    "read through": "通过",
    "none unless this slice changes schema/data backfill": "除非该子需求改变表结构或数据回填，否则无需迁移",
    "none unless": "除非",
    "preserve existing failure behavior": "保持既有失败处理方式",

...
- Bugfix: `Runtime intake -> requirement-document-ingestor -> spec-governor -> requirement-question-governor -> technical-design-governor -> design-architecture-reviewer -> test-design-governor -> test-data-governor -> delivery-plan-templates -> traceability-governor initial pass -> delivery-plan-reviewer -> Runtime/Harness design -> git-worktree-governor -> edit-readiness-governor -> Runtime pre-edit`; API/data/UI/cross-repo/MQ/async/scheduler/task/job/cache/integration/permission/security signals elevate above the light path.
- Standard feature: `Runtime intake -> requirement-document-ingestor -> Harness source-location when repository-backed -> spec-governor -> requirement-question-governor -> domain-model-governor -> architecture-framing-governor -> specialty design governors -> technical-design-governor -> architecture-design-governor -> design-architecture-reviewer -> test-design-governor -> test-data-governor -> delivery-plan-templates -> cross-repo-planner when needed -> traceability-governor initial pass -> delivery-plan-reviewer -> Runtime/Harness design`.
...
- Release readiness: start from `implementation-completion-gate`, then `post-change-skill-sync`, Runtime/Harness post-implementation, `code-review-gate`, `test-evidence-gate`, post-implementation traceability, Runtime/Harness pre-push, environment/UAT/release-change evidence, Runtime provider attestations, and `release-evidence-binder`.
...
Harness checkpoints are provided by `auto-runner` and configured through `config/harness-policy.example.yaml`; they are workflow stages rather than separate skills, so they do not add another always-visible skill description.
Agent Runtime is also embedded in `auto-runner` and configured through `config/agent-runtime-policy.example.yaml`. It adds execution authorization and verifiable event/checkpoint evidence without consuming another skill description slot.
